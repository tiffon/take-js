...
function ensureCherio(elm) {
    if (elm instanceof cheerio) {
        return elm;
    }
    return cheerio(elm);
}
...
function makeCssQuery(selector) {
    return function cssQuery(elm) {
        return ensureCherio(elm).find(selector);
    };
}
